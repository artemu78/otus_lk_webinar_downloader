...
7. На странице проекта Hexlet находит ссылку на GitHub в DOM и предоставляет те же четыре действия, используя отдельный локальный корень.
...
На странице `https://ru.hexlet.io/tutorship/project_members/` отображается тот же
набор действий. Расширение берёт `href` первой ссылки на GitHub-репозиторий и
строит путь `DEFAULT_ALLOWED_ROOT_HEXLET/homeworks/<owner>/<repository>`.
Абсолютный путь можно изменить и сохранить в поле «Локальная папка работы»;
он кэшируется отдельно для каждого `owner/repository`. Кнопка скачивания передаёт
ссылку локальному серверу напрямую, без OTUS API и OpenRouter, после чего сервер
клонирует репозиторий по SSH через `GITHUB_SSH_HOST`.
...
соответствующего корня: `DEFAULT_ALLOWED_ROOT` для OTUS или
`DEFAULT_ALLOWED_ROOT_HEXLET` для Hexlet. Подробности и настройки находятся в
...
Расширение запрашивает доступ к активной вкладке, записи в буфер обмена, загрузкам Chrome, хранилищу данных сессии, страницам `https://otus.ru/*`, локальному серверу `http://127.0.0.1:8765/*` и Google Таблицам. Доступ к DOM страницы Hexlet действует только для активной вкладки после нажатия на расширение. Контент-скрипт для Google Таблиц только показывает инструкцию по вставке данных в новой таблице, открытой расширением.
